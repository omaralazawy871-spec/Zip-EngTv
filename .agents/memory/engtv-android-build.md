...
**The manually-installed Android SDK at `/home/runner/android-sdk` does not persist across sessions** (it's outside the workspace/project dir) — expect to redo step 3 (cmdline-tools download + `sdkmanager` install) at the start of each new session before building. The JDK (`java-graalvm22.3` module) and system `gradle` package, and the release keystore at the workspace root, do persist.
